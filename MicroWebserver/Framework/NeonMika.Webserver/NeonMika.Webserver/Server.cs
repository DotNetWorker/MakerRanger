...
namespace NeonMika.Webserver
{
   ...
   // public delegate void JSONResponseMethod(Request e, JsonArray results);
    ...
    public class Server
    {
        public event FileProcessed AfterFileRecieved;
        public event SetSettingsValue AfterSettingsValueSet;
        ...
        public Server(int port = 80, bool DhcpEnable = true, string ipAddress = "", string subnetMask = "", string gatewayAddress = "", string networkName = "NETDUINOPLUS")
        {
        ...
        private void ResponseListInitialize()
        {
            ...
            AddResponse(new XMLResponse("secret", new XMLResponseMethod(Secret)));
            ...
            AddResponse(new XMLResponse("settings", new XMLResponseMethod(SetSettings)));
        ...
        private void Secret(Request e, Hashtable results)
        {
            foreach (string item in FileReader.SDFileReader.GetSecretNumbers(@"SD\SecretNumber.txt"))
            {
                results.Add("secret", item);
            }
        ...
        private void SetSettings(Request e, Hashtable ret)
        {
            if (e.GetArguments.Contains("MaxQtyValue") && e.GetArguments.Contains("MaxGuessValue"))
            {
                try
                {
                    AfterSettingsValueSet(e.GetArguments["MaxQtyValue"].ToString(),e.GetArguments["MaxGuessValue"].ToString());
                }
                catch (Exception ex)
                {
                    Debug.Print(ex.ToString());
                    ret = XMLResponse.GenerateErrorHashtable("MaxQtyValue", ResponseErrorType.InternalOperationError);
                }
            }
            else
            {
                ret = XMLResponse.GenerateErrorHashtable("MaxQtyValue or MaxGuessValue", ResponseErrorType.ParameterMissing);
            }
